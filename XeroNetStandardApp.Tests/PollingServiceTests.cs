...
namespace XeroNetStandardApp.Tests
{
    public class PollingServiceTests
    {
        ...
        public async Task AssetsEndpoint_UsesAssetsService()
        {
            ...
            var tokenService = new TokenService(new NullDataProtectionProvider());
            tokenService.StoreToken(new XeroOAuth2Token { AccessToken = "t", ExpiresAtUtc = DateTime.UtcNow.AddHours(1) });
